...
describe('parser-version', () => {
  ...
  describe('exists()', () => {
    ...
    it('constructs correct Maven Central URL', function () {
      this.timeout(15000);
      // Test that the URL format is correct by checking a known version
      // The exists() function should check:
      // https://repo.maven.apache.org/maven2/org/eolang/eo-maven-plugin/{version}/eo-maven-plugin-{version}.pom
      const exists = parserVersion.exists('0.28.11');
    ...
    it('handles network errors gracefully', function () {
      this.timeout(15000);
      // This test assumes exists() will return true on network errors
      // We can't easily simulate network errors in tests, but we can verify
      // that the function doesn't throw on valid input
      assert.doesNotThrow(() => {
